...
export const getHourDetails = async (req, res) => {
  try {
    ...
    if (!["working", "extra", "total"].includes(type)) {
      return res.status(400).json({ success: false, message: "Invalid type. Must be working, extra, or total." });
    }
    const details = await reportService.getHourDetails(req.user.id, req.user.role, type);
